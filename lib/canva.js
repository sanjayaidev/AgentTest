...
async function listDesigns(accessToken, { limit = 50 } = {}) {
  const allDesigns = [];
  ...
  do {
    ...
    if (data.items && Array.isArray(data.items)) {
      allDesigns.push(...data.items);
    }
    
    continuation = data.continuation || null;
    
    // Safety limit to prevent infinite loops
    if (allDesigns.length > 500) {
      console.log('[DEBUG] Reached safety limit of 500 designs, stopping pagination');
      break;
    }
  } while (continuation);
  
  console.log('[DEBUG] Total designs fetched:', allDesigns.length);
  return allDesigns;
}
