...
async function listDesigns(accessToken, { limit = 5, continuation = null } = {}) {
  const url = `/designs?limit=${limit}${continuation ? `&continuation=${continuation}` : ''}`;
  console.log('[DEBUG] Calling Canva API:', `${API_BASE}${url}`);
  const data = await canvaApiRequest(accessToken, 'GET', url);
  console.log('[DEBUG] Canva API response for designs - items count:', data.items?.length || 0, 'continuation:', data.continuation ? 'yes' : 'no');
  
  const designs = data.items && Array.isArray(data.items) ? data.items : [];
  console.log('[DEBUG] Designs fetched:', designs.length, 'has more:', !!data.continuation);
...
async function getDesignPages(accessToken, designId) {
  // The Canva API supports offset/limit (default limit is 50, max 200).
  // We loop through all pages so designs with >50 pages are fully returned.
  const limit = 200;
  let offset = 1;
  let allPages = [];

  while (true) {
    const url = `/designs/${designId}/pages?offset=${offset}&limit=${limit}`;
    console.log('[DEBUG] Fetching pages for design:', designId, 'offset:', offset);
    const data = await canvaApiRequest(accessToken, 'GET', url);
    const items = data.items || [];
    console.log('[DEBUG] Pages fetched in this batch:', items.length);
