...
async function listDesigns(accessToken, { limit = 50 } = {}) {
  const url = `${API_BASE}/designs?limit=${limit}`;
  console.log('[DEBUG] Calling Canva API:', url);
  const data = await canvaApiRequest(accessToken, 'GET', `/designs?limit=${limit}`);
  console.log('[DEBUG] Canva API response for designs:', JSON.stringify(data, null, 2));
  // Canva API returns designs in 'items' array, not 'designs'
  return data.items || [];
}
